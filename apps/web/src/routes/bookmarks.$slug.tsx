...
function RouteComponent() {
  const { slug } = useParams({ from: "/bookmarks/$slug" });
  ...
  const [foldersState, setFolders] = useAtom(foldersAtom);
  ...
  useEffect(() => {
    ...
    setFolders((prev: any) =>
      prev === fetchedFolders ? prev : fetchedFolders
    );

    const selected = fetchedFolders.find((f: any) => f.slug === slug);
    if (selected && selected.id !== currentFolder?.id) {
      setCurrentFolder(selected);
      const prevSaved = localStorage.getItem("currentFolderId");
      if (prevSaved !== selected.id) {
        localStorage.setItem("currentFolderId", selected.id);
      }
    }
  }, [fetchedFolders, slug, setFolders, setCurrentFolder, currentFolder]);

  useEffect(() => {
    const list = (foldersState as any[]) || [];
    if (list.length === 0) return;
    const exists = !!list.find((f: any) => f.slug === slug);
    if (!exists) {
      const first: any = list[0];
      if (first?.slug && first.slug !== slug) {
        navigate({ to: "/bookmarks/$slug", params: { slug: first.slug } });
      }
    }
  }, [foldersState, slug, navigate]);

  // Defer UI decisions until after all hooks are declared
  ...
  const { mutate: addBookmarkMutation, isPending: isAddingBookmark } =
    useMutation({
      ...
      onMutate: async (newBookmark) => {
        ...
        const tempTitle = (() => {
          try {
            const hostname = new URL(newBookmark.url).hostname.replace(
              /^www\./,
              ""
            );
            return hostname.split(".")[0];
          } catch {
            return newBookmark.url;
          }
        })();
  ...
  if (foldersLoading) {
    return (
      <div className="p-3 sm:p-5 md:p-8">
        <div className="w-full px-1 sm:px-4 pb-20 sm:w-[40rem] md:w-[48rem] lg:w-[50rem]">
          <SkeletonList viewStyle="compact" />
        </div>
      </div>
    );
  }
  if (foldersFetched && (!foldersState || foldersState.length === 0)) {
    return (
      <div className="p-3 sm:p-5 md:p-8">
        <CreateFirstFolder />
      </div>
        ...
        <div className="w-full px-1 sm:px-4 pb-20 sm:pb-32 sm:w-[40rem] md:w-[48rem] md:px-0 lg:w-[50rem]">
          ...
          >
            <input
              ...
              placeholder={
                !foldersState || foldersState.length === 0
          ...
          <motion.ul>
            {filteredBookmarks.length === 0 &&
              ...
              foldersState &&
              foldersState.length > 0 && <SkeletonList viewStyle={viewStyle} />}
            ...
            {(!foldersState || foldersState.length === 0) && foldersFetched && (
              <CreateFirstFolder />
            )}
